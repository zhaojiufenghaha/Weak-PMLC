...
class BERTDataIter():
    ...
    def __init__(self, data_path: str, tokenizer: BertTokenizer, batch_size: int = 64, shuffle: bool = True,
                 max_len: int = 128, label_mask_type=None, task="train", num_labels=69, mask_order="random",
    ...
    def reset(self):
        logger.info("dataiter reset, 读取{}数据".format(self.task))
    ...
    def _read_data(self, data_path):  # 读取数据并转为id形式，[  [ids1,ids2,label]  ]
        """
         读取数据并转为id形式
        :param data_path:
    ...
    def get_batch_data(self):
        ...
        if len(batch_data) < 1:
            return None
        if self.label_mask_type is not None:  # 使用LabelMaskModel模型去跑
            if self.task == "train":  # 使用训练集
                if self.label_mask_type == "part":  # 不是很懂
                    num_mask = random.randint(1, self.num_labels)  # 返回1和self.num_labels之间的任意整数，然后去mask
                    if self.mask_order == "random":
                        masked_labels_list = [random.sample(self.all_labels, num_mask) for _ in range(len(batch_data))]
                        # masked_labels_list不是很懂 来做什么
...
def _get_bert_input_single_sen(data, max_len, tokenizer, mlm_ratio=0.15):
    ...
    for ids, label in data:
        res = tokenizer.prepare_for_model(ids, pair_ids=None, max_length=max_len, add_special_tokens=True,truncation=True)
...
def get_labelbert_input_single_sen(data, max_len, tokenizer, masked_labels_list=None, pred_labels_list=None,
    ...
    num_add_tokens_per_label = 1 + num_pattern_begin + num_pattern_end  # 不是很懂
