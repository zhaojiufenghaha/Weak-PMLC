...
class WeakPMLCModel(BertPreTrainedModel):

    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        # self.bert = BertModel(config, add_pooling_layer=False)
        self.bert = BertModel.from_pretrained('wobert_model_path')
    ...
    def forward(self, input_ids, pred_mode, attention_mask=None, token_type_ids=None,
        ...
        if pred_mode == "classification":
            ...
            outputs = self.classifier(trans_states)
        elif pred_mode == "mlm":
            outputs = self.cls(last_hidden_states)
        else:
